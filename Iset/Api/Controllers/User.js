...
/*export const CreateEventWithUserId=async(req,res,next)=>{
  ...
  try {
    ...
    console.log(JSON.stringify(AddUser._id))
    try {
      await Event.findByIdAndUpdate({_id:req.params.idEvent},{$push:{users:AddUser._id}}).exec();
    } catch (error) {
      next(error)
    }
...
export const CreateEventWithUserId=async(req,res,next)=>{
  ...
  const newUser=new User(req.body);
  try {
    const anthorUser=await newUser.save();
    try {
      await Event.findOneAndUpdate(EventId,{$push:{users:anthorUser._id}});
    } catch (error) {
      next(error);
    }
    res.status(200).json(anthorUser);
  } catch (error) {
    next(error);
  }
}
